...
class StrategyZSTupo(Strategy):
    ...
    def __init__(self, pre_zf_rate=None, zs_zd_days=None, ema_params=None, max_loss_rate=None):
        super().__init__()
    ...
    def open(self, code, market_data: MarketDatas, poss: Dict[str, POSITION]) -> List[Operation]:
        ...
        info = {
            ...
            'zx_into_line_mmd_bc': '/'.join(xd_zs_lv0.lines[0].line_mmds() + xd_zs_lv0.lines[0].line_bcs()),
        ...
        if opt_direction == 'buy' and kline_lv1.c > ema_idx[0][-1] and \
            ...
            opts.append(Operation(
                'buy', '3buy', loss_price, info, f'价格突破 zg {xd_zs_lv0.zg}，做多买入 {open_pos_rate}',
        ...
        if opt_direction == 'sell' and kline_lv1.c < ema_idx[0][-1] and \
            ...
            info['open_pos_rate'] = open_pos_rate  # 记录开仓占比
            opts.append(Operation(
                'buy', '3sell', loss_price, info, f'价格突破 zd {xd_zs_lv0.zd}，做空卖出 {open_pos_rate}',
                pos_rate=open_pos_rate
            ))
    ...
    def close(self, code, mmd: str, pos: POSITION, market_data: MarketDatas) -> [Operation, None, List[Operation]]:
        ...
        open_days = len([_k for _k in cd_lv1.get_klines() if _k.date > pos.info['open_date']])
        if open_days >= 5:
            if ('buy' in mmd and kline_lv1.c < kline_lv1.o) or ('sell' in mmd and kline_lv1.c > kline_lv1.o):
                opts.append(Operation(
                    'sell', mmd,
                    msg=f'开仓后 5 日，进行平仓三分之一 ({pos.info["open_date"]} / {kline_lv1.date} [{open_days}])',
                    pos_rate=round(pos.info['open_pos_rate'] * 0.3, 2), key='5'  # 设置一个Key，后续接到相同的key，则不进行处理，这样保证平仓只执行一次
                ))
            # 止损价格移动到入场位置，关闭风险
        ...
        if 'buy' in mmd and kline_lv1.c < ema_idx_10[-1] and kline_lv1.c < kline_lv1.o:
            opts.append(Operation(
                ...
                pos_rate=round(pos.info['open_pos_rate'] * 0.3, 2), key='10'
            ))
