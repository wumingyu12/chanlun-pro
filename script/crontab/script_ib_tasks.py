...
def run_tasks(client_id: int):
    ...
    def get_contract_by_code(code: str):
        ...
        if '_IND_' in code:
            ...
            # 读取代码所属交易所信息，在合约中添加
            path = os.path.split(os.path.realpath(__file__))[0] + '/us_exchange_stocks.json'
            with open(path, 'r') as fp:
                exchange_stocks: dict = json.load(fp)
            contract = ib_insync.Stock(symbol=code, exchange='SMART', currency='USD')
            if code in exchange_stocks['NASDAQ']:
                contract.primaryExchange = 'NASDAQ'
            elif code in exchange_stocks['NYSE']:
                contract.primaryExchange = 'NYSE'
            elif code in exchange_stocks['AMEX']:
